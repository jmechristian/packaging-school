...
const PathCompleteModal = ({ path, onClose, credential }) => {
  ...
  const handleAddToLinkedIn = async () => {
    if (linkedin) {
      ...
      window.open(
        `https://www.linkedin.com/profile/add?startTask=CERTICATION_NAME&certId=${credential.id}&certUrl=${credential.url}&isFromA2p=true&issueMonth=${issueMonth}&issueYear=${issueYear}&name=${credential.name}&organizationId=6672102`,
        '_blank'
      );
    } else {
