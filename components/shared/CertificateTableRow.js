...
const CertificateTableRow = ({
  ...
  return (
    <>
      <tr
        className={`border-b border-slate-200 transition-colors group bg-clemson/5 ${
      ...
      >
        <td className={`sticky left-0 z-[1] px-2 sm:px-3 py-2.5 sm:py-3 text-xs font-mono text-slate-600 align-top whitespace-nowrap shadow-[2px_0_4px_-2px_rgba(0,0,0,0.08)] ${
          isNavigating ? 'bg-slate-100' : 'bg-clemson/5 group-hover:bg-slate-50'
        }`}>
          {certificate.courseId}
        </td>
        <td className='px-2 sm:px-3 py-2.5 sm:py-3 align-top min-w-0'>
          <div className='font-semibold text-slate-900 text-sm sm:text-base leading-tight break-words'>
            {certificate.title}
        ...
        <SubheadlineCell content={certificate.description || ''} className='py-2.5 sm:py-3' />
