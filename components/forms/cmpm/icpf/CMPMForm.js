...
import { saveIcpfCmpmForm, getAWSUser } from '../../../../helpers/api';
  ...
  const submitFormToAWS = async () => {
    ...
    await API.graphql({
      ...
      variables: {
        input: {
          ...
          icpfCmpmFormUserId: awsUser && awsUser.id ? awsUser.id : null,
          ...
          fullTime: methods.getValues('fullTime'),
          ...
          contactConsent: methods.getValues('contactConsent'),
          videoLink: methods.getValues('videoLink'),
          manualVideoLink: methods.getValues('manualVideoLink'),
          status: 'SUBMITTED',
        },
      },
    });
    ...
    if (awsUser) {
      await API.graphql({
        query: updateUser,
        variables: {
          input: { id: awsUser.id, icpfCmpmFormID: id },
        },
      });
