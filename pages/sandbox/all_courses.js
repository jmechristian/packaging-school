...
const Page = () => {
  ...
  return (
    <>
      ...
      />
      <div className='w-full max-w-7xl mx-auto px-3 xl:px-0'>
        <div className='w-full flex flex-col gap-0'>
          {/* HEADING */}
          <div className='w-full pb-4 border-b border-b-black flex flex-col lg:flex-row gap-4 lg:gap-0 lg:justify-between items-center'>
            <div className='h3-base'>Browse Full Catalog</div>
            <div className='w-full border border-slate-400 p-1 lg:!w-1/2 rounded-md'>
          ...
          {isLoading ? (
            <div className='grid lg:grid-cols-3 xl:grid-cols-4 md:grid-cols-2 gap-y-8 gap-x-7 mt-5'>
              ...
              {[...Array(8)].map((_, index) => (
                <div
                  key={index}
                  className='bg-gray-200 h-40 animate-pulse'
                ></div>
              ))}
            </div>
          ) : (
            <>
              ...
              <div className='grid grid-cols-3 lg:mb-5 border-b-2 border-b-black py-6 gap-2.5'>
                ...
                <div className='col-span-3 flex justify-end lg:justify-start gap-5 relative w-full'>
                  <div className='hidden lg:flex w-full lg:flex-wrap gap-2 col-span-3'>
                    {categoryMenu.slice(0, 8).map((cat) => (
                      <div
                        key={cat.value}
                        className={` transition-all ease-in flex w-fit items-center justify-between px-2 cursor-pointer ${
              ...
              <div className='flex flex-wrap lg:mb-5 border-b border-b-slate-900 pb-6 gap-2.5 md:mt-1'>
                {updateCategoryMenu.map((cat) => (
                  <div
                    key={cat.value}
                    className='flex items-center gap-2 border-slate-300 rounded border bg-neutral-200 hover:bg-base-light px-3 py-1.5 text-sm font-semibold cursor-pointer'
              ...
              isTable ? (
                ...
                <div className='grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-y-6 gap-x-0 mt-5'>
                  {sortedCertificates &&
                    sortedCertificates.length > 0 &&
                    [...sortedCertificates].map((cert) => (
                      <CertCard
